...
//
	include("generated_file_banner.php");
//
// Asterisk Templates - doesn't seem to work when #included so we must put it in-line
//
	$OUT = file_get_contents("/etc/asterisk/sark_sip_templates.conf");	
	 
	$sql = "SELECT * FROM IPphone order by pkey";
    foreach ($dbh->query($sql) as $row) {
		if ($row['active'] == "YES") {       
			if ($row['technology'] == "SIP") {
// TODO!! - add transport selection here
				if ($row['transport'] == "tcp") {
					$OUT .= "[" . $row['pkey'] . "](sark-sip-std-ext,sark-sip-std-codec,sark-sip-tcp) \n";
				}
 				else if ($row['transport'] == "tls") {
					$OUT .= "[" . $row['pkey'] . "](sark-sip-std-ext,sark-sip-std-codec,sark-sip-tls) \n";
				}
				else {
					$OUT .= "[" . $row['pkey'] . "](sark-sip-std-ext,sark-sip-std-codec) \n";
				} 
