...
//	$debian = $this->helper->request_syscmd( "/bin/cat /etc/debian_version" );
//	$debian_rel = trim($debian);
	$shorewall = $this->helper->request_syscmd( "/sbin/shorewall version" );
	$shorewall_rel = trim($shorewall);
	if ($shorewall_rel >= 4.6) {
 ...
 */
    ...
    $this->myPanel->aLabelFor('fqdnprov');
    $this->myPanel->selected = $global['FQDNPROV'];
    $this->myPanel->popUp('ACL', array('NO','YES')); 
