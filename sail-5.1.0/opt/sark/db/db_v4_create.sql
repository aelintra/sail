...
ACL TEXT,                           -- ON/OFF 
AGENTSTART TEXT,                    -- Agent start number
...
CLUSTER TEXT DEFAULT 'default',     -- defaut cluster name
...
EXTBLKLST TEXT,						-- YES/NO loads voipbl.com external SIP blacklist into an ipset
...
FQDN TEXT,							-- FQDN V5+
FQDNINSPECT TEXT,					-- Require FQDN in INVITE and REGISTER (Debian 8+)
FQDNPROV TEXT,						-- use FQDN in remote provisioning true/false
...
LOGLEVEL TEXT,                      -- internal log level
...
SIPFLOOD TEXT,						-- detect SIP flood YES/NO
...
SYSOP TEXT,                         -- system operator real extension
...
description TEXT,
cluster TEXT,
greetnum TEXT,						-- greeting number to play
listenforext TEXT,
name TEXT,
...
/* harvest of phones which have issued SIP multicast requests */ 
...
selection TEXT,						-- user class (enduser|poweruser|tenant|admin)
z_created datetime,
z_updated datetime,
z_updater TEXT DEFAULT 'system',
UNIQUE (pkey),
...
/*  deleted in 5.x
CREATE TRIGGER Device_atl_insert AFTER INSERT ON Device_atl
BEGIN
   UPDATE Device_atl set z_created=datetime('now'), z_updated=datetime('now') where pkey=new.pkey;
   INSERT INTO master_audit(act,owner,relation,tstamp) VALUES ('INSERT', new.pkey, 'Device_atl', datetime('now'));   
END;
CREATE TRIGGER Device_atl_update AFTER UPDATE ON Device_atl
BEGIN
   UPDATE Device_atl set z_updated=datetime('now') where pkey=new.pkey;
   INSERT INTO master_audit(act,owner,relation,tstamp) VALUES ('UPDATE', new.pkey, 'Device_atl', datetime('now'));
END;
CREATE TRIGGER Device_atl_delete AFTER DELETE ON Device_atl
BEGIN
   INSERT INTO master_audit(act,owner,relation,tstamp) VALUES ('DELETE', old.pkey, 'Device_atl', datetime('now'));
END;
*/
...
CREATE TRIGGER netphone_insert AFTER INSERT ON netphone
BEGIN
   UPDATE netphone set z_created=datetime('now'), z_updated=datetime('now') where pkey=new.pkey;
   INSERT INTO master_audit(act,owner,relation,tstamp) VALUES ('INSERT', new.pkey, 'netphone', datetime('now'));   
END;
CREATE TRIGGER netphone_update AFTER UPDATE ON netphone
BEGIN
   UPDATE netphone set z_updated=datetime('now') where pkey=new.pkey;
   INSERT INTO master_audit(act,owner,relation,tstamp) VALUES ('UPDATE', new.pkey, 'netphone', datetime('now'));
END;
CREATE TRIGGER netphone_delete AFTER DELETE ON netphone
BEGIN
   INSERT INTO master_audit(act,owner,relation,tstamp) VALUES ('DELETE', old.pkey, 'netphone', datetime('now'));
END;
...
CREATE TRIGGER tt_help_core_insert AFTER INSERT ON tt_help_core
BEGIN
   UPDATE tt_help_core set z_created=datetime('now'), z_updated=datetime('now') where pkey=new.pkey;
   INSERT INTO master_audit(act,owner,relation,tstamp) VALUES ('INSERT', new.pkey, 'tt_help_core', datetime('now'));   
END;
CREATE TRIGGER tt_help_core_update AFTER UPDATE ON tt_help_core
BEGIN
   UPDATE tt_help_core set z_updated=datetime('now') where pkey=new.pkey;
   INSERT INTO master_audit(act,owner,relation,tstamp) VALUES ('UPDATE', new.pkey, 'tt_help_core', datetime('now'));
END;
CREATE TRIGGER tt_help_core_delete AFTER DELETE ON tt_help_core
BEGIN
   INSERT INTO master_audit(act,owner,relation,tstamp) VALUES ('DELETE', old.pkey, 'tt_help_core', datetime('now'));
END;

/* deleted in 5.x
CREATE TRIGGER vendorxref_insert AFTER INSERT ON vendorxref
BEGIN
   UPDATE vendorxref set z_created=datetime('now'), z_updated=datetime('now') where pkey=new.pkey;
   INSERT INTO master_audit(act,owner,relation,tstamp) VALUES ('INSERT', new.pkey, 'vendorxref', datetime('now'));   
END;
CREATE TRIGGER vendorxref_update AFTER UPDATE ON vendorxref
BEGIN
   UPDATE vendorxref set z_updated=datetime('now') where pkey=new.pkey;
   INSERT INTO master_audit(act,owner,relation,tstamp) VALUES ('UPDATE', new.pkey, 'vendorxref', datetime('now'));
END;
CREATE TRIGGER vendorxref_delete AFTER DELETE ON vendorxref
BEGIN
   INSERT INTO master_audit(act,owner,relation,tstamp) VALUES ('DELETE', old.pkey, 'vendorxref', datetime('now'));
END;
*/
...
CREATE TRIGGER callback_xref_insert AFTER INSERT ON callback
BEGIN
	INSERT INTO master_xref(pkey, relation) VALUES (new.pkey, 'callback');
END;
CREATE TRIGGER callback_xref_delete AFTER DELETE ON callback
BEGIN
   DELETE from master_xref WHERE pkey=old.pkey AND relation='callback'; 
END;
CREATE TRIGGER callback_update_key AFTER UPDATE OF pkey ON callback
BEGIN
   UPDATE master_xref set pkey=new.pkey where pkey=old.pkey AND relation='callback';
END;
