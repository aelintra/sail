 ...
 */ 
		if ($this->smartlink) {
			...
			$extlen = $res['EXTLEN'];
			$user = $this->dbh->query("SELECT cluster FROM user WHERE pkey = '" . $_SERVER['REMOTE_USER'] . "'")->fetch(PDO::FETCH_ASSOC);
		}
		...
		for ($i = 0; $i < $this->span; $i++) {	
			if ($this->smartlink) {
				...
				if (isset($extfetch['pkey'])) {
					if ( $user['cluster'] ==  $extfetch['cluster'] || $_SERVER['REMOTE_USER'] == 'admin' ) {
