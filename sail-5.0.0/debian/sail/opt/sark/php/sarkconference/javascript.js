...
//		   mcastlport: "Enter a port number between 0 and 65535"
	...
	$('#conferencetable').dataTable ( {
		...
		"fnRowCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull ) {
          $('td:eq(1),td:eq(2),td:eq(3),td:eq(4)', nRow).addClass( "bluetags" );
			...
			"aoColumns": [
				null,		// pkey
				{
					tooltip: 'Click to select conference type',
					event: 'click',
					type: 'select',
                    onblur: 'cancel',
                    submit: 'Save',
				...
				},		// pin
				{
					...
					event: 'click',
					submit:'Save',
					tooltip: 'Click to set admin pin',
					onblur: 'cancel',	
					placeholder: 'Null'
				},		// admin pin				
				{
        ...
        $(".dataTables_scrollBody").find("tr").find('td:eq(5):not(:contains("free"))').parent().css('backgroundColor', 'yellow') ;          
 });
