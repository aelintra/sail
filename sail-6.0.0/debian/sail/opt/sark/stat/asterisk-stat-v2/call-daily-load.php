...
<!-- ** ** ** ** ** Part for the research ** ** ** ** ** -->
	...
	<INPUT TYPE="hidden" NAME="posted" value=1>
		<table class="bar-status" width="75%" border="0" cellspacing="1" cellpadding="2" align="center">
        		...
        		<td align="left" bgcolor="#CC0000">					
      			...
      			<td align="left" bgcolor="#CCCCCC">
					<table width="100%" border="0" cellspacing="0" cellpadding="0" bgcolor="#CCCCCC"><tr><td>
	  				<b><font face="arial" size="1">From : </b><select name="fromstatsday_sday">
					<? 
						for ($i=1;$i<=31;$i++){
			...
			<tr>
        		<td class="bar-search" align="left" bgcolor="#000"> </td>
...
<tbody><tr><td bgcolor="#000000">			
	...
	<br>
 	<IMG SRC="graph_statbar.php?min_call=<?=$min_call?>&fromstatsday_sday=<?=$fromstatsday_sday?>&days_compare=<?=$days_compare?>&fromstatsmonth_sday=<?=$fromstatsmonth_sday?>&dsttype=<?=$dsttype?>&sourcetype=<?=$sourcetype?>&clidtype=<?=$clidtype?>&channel=<?=$channel?>&resulttype=<?=$resulttype?>&dst=<?=$dst?>&src=<?=$src?>&clid=<?=$clid?>&userfieldtype=<?=$userfieldtype?>&userfield=<?=$userfield?>&accountcodetype=<?=$accountcodetype?>&accountcode=<?=$accountcode?>" ALT="Stat Graph">
...
<br/>
	...
	<FORM METHOD=POST ACTION="graph_hourdetail.php?posted=<?=$posted?>&min_call=<?=$min_call?>&fromstatsday_sday=<?=$fromstatsday_sday?>&days_compare=<?=$days_compare?>&fromstatsmonth_sday=<?=$fromstatsmonth_sday?>&dsttype=<?=$dsttype?>&sourcetype=<?=$sourcetype?>&clidtype=<?=$clidtype?>&channel=<?=$channel?>&resulttype=<?=$resulttype?>&dst=<?=$dst?>&src=<?=$src?>&clid=<?=$clid?>&userfieldtype=<?=$userfieldtype?>&userfield=<?=$userfield?>&accountcodetype=<?=$accountcodetype?>&accountcode=<?=$accountcode?>" target="superframe">		
	<!-- ** ** ** ** ** HOURLY LOAD ** ** ** ** ** -->
...
<br>
<center>
    <iframe name="superframe" src="graph_hourdetail.php?posted=<?=$posted?>&min_call=<?=$min_call?>&fromstatsday_sday=<?=$fromstatsday_sday?>&days_compare=<?=$days_compare?>&fromstatsmonth_sday=<?=$fromstatsmonth_sday?>&dsttype=<?=$dsttype?>&sourcetype=<?=$sourcetype?>&clidtype=<?=$clidtype?>&channel=<?=$channel?>&resulttype=<?=$resulttype?>&dst=<?=$dst?>&src=<?=$src?>&clid=<?=$clid?>&userfieldtype=<?=$userfieldtype?>&userfield=<?=$userfield?>&accountcodetype=<?=$accountcodetype?>&accountcode=<?=$accountcode?>" BGCOLOR=white	width=770 height=800 marginWidth=0 marginHeight=0  frameBorder=0  scrolling=yes>
