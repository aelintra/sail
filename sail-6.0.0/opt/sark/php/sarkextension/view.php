...
/*** table rows ****/
	...
	foreach ($rows as $row ) {
    	...
    	echo '<td  class="icons" title = "' . $display_ipaddr . '" >' . $display  . '</td>' . PHP_EOL;
		...
		echo '<td class="icons">' . $row['transport'] . '</td>' . PHP_EOL;
...
//			}
	...
	if (!empty($extension['macaddr'])) {
		if (!empty($expand_prov)) { 
			echo '<div style="display:none"><div id="provgen"><div class="fancybox" style="font-size:12px" ><pre>' . htmlentities($expand_prov) . '</pre></div></div></div>'  . PHP_EOL;
		}
	}	
 ...
 */  
	...
	if ($extension['technology'] == 'SIP' ) {
		...
		$this->myPanel->aLabelFor('provisionwith');
		$this->myPanel->selected = $extension['provisionwith'];
		$this->myPanel->popUp('provisionwith', array('IP','FQDN'));	
		
		$this->myPanel->aLabelFor('protocol');
		$this->myPanel->selected = $extension['protocol'];
		$this->myPanel->popUp('protocol', $protocol);		
		
		$this->myPanel->aLabelFor('transport');
		$this->myPanel->selected = $extension['transport'];
		$this->myPanel->popUp('transport', array('udp','tcp','tls'));		
	}	
