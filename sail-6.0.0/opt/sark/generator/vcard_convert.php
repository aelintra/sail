...
/*
 ...
 +-----------------------------------------------------------------------+
 * Heavily hacked by CoCo to provide an experimental Vcard to LDAP       |
 * converter for SARK.													 |
 * Apologies to the original authors for their fine work				 | 
 *-----------------------------------------------------------------------+
 Call it with
  
 php /opt/sark/generator/vcfconvert.php -f ldap -b "ou=contacts,dc=sark,dc=local" -o /tmp/newldif  /tmp/contacts.vcf
 * 
 *
 Then you can use ldapadd -l to upload the new rows.
 * 
 ldapadd -H ldap://localhost -xc -D "cn=admin,dc=sark,dc=local" -f /tmp/newldif -w {adminpwd}
 * 
*/
...
class vcard_convert extends Contact_Vcard_Parse
{
	...
	function toLdif($identifier="", $attrib=null, $encoding="UTF-8")
	{
		...
		foreach($this->cards as $card)
		{
			if ($this->mailonly && empty($card->email) && empty($card->email2))
				continue;
			if ($this->phoneonly && ! is_numeric($card->home['phone']) && ! is_numeric($card->work['phone']) && ! is_numeric($card->mobile) )
				continue;
...
//			$a_out['objectclass'] = array('top', 'person', 'organizationalPerson', 'inetOrgPerson', 'mozillaAbPersonAlpha');
			$a_out['objectclass'] = array('top', 'person', 'organizationalPerson', 'inetOrgPerson');
...
*/
			if ($this->normalize_phone($card->home['phone']))
				$a_out['homePhone'] = $this->normalize_phone($card->home['phone']);
			if ($this->normalize_phone($card->mobile))
...
*/
			if ($this->normalize_phone($card->work['phone']))
