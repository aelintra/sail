...
// find the next available ext#
	...
	echo '<p><textarea name="txtmacblock" class="w3-padding w3-margin-bottom w3-small w3-card-4 longdatabox"></textarea>';
...
//print_r($_REQUEST); 
	...
	if ( $_POST['extchooser'] == 'Provisioned') {
    	$this->validator->addValidation("macaddr","regexp=/^[0-9a-fA-F]{12}|\s*$/","Mac address is invalid");  
    	$this->validator->addValidation("macaddr","req","Please provide MAC address"); 
    }

    //Now, validate the form
    if ( ! $this->validator->ValidateForm()) {
    	$this->error_hash = array_merge($this->error_hash,$this->validator->GetErrors());
		$this->message = "Validation Errors!";
		return;
	}
		
	$tuple['pkey'] 			= strip_tags($_POST['pkey']);
	if ( strlen($tuple['pkey']) != $extlen ) {
		$this->error_hash['KEY'] = "Extension length is incorrect";
    ...
    switch ($myChooser) {			
		case 'Provisioned':				
			if (isset($_POST['macaddr'])) {	
				...
				$res = $this->getVendorFromMac($tuple['macaddr']);
				if ($res) {
					if ($this->checkThisMacForDups($tuple['macaddr'])) {
						$this->error_hash['MAC'] = "MAC address already exists";
						$this->invalidForm = True;
						return -1;
					}
					else {
					...
					$this->error_hash['MAC'] = "Invalid or unsupported MAC address";
					$this->invalidForm = True;
					return -1;
				} 
			}
			else {
				$this->error_hash['MAC'] = "No MAC address entered";
				$this->invalidForm = True;
				return -1;
			}
...
*/
/*
		if ($tuple['transport'] == 'tls') {
			$tuple['sipiaxfriend'] .= "\n#include sark_sip_tls.conf";
		}
		elseif ($tuple['transport'] == 'tcp') {
			$tuple['sipiaxfriend'] .= "\n#include sark_sip_tcp.conf";
		}
*/
